...
FFT_handler::FFT_handler(int raw_data_sample_nr, int windows_nr, float *raw_data,float *fft_amps)
{// Constructor
...
void FFT_handler::Windowing(int window)
	...
	for (int j=0;j<this->_windows_nr;j++){
		...
		for (int i = 0; i < (this->_window_sample_nr >> 1); i++) {
			...
			case FFT_WIN_TYPE_TRIANGLE: // triangle (Bartlett)
				...
				#else
				weighingFactor = 1.0 - ((2.0 * std::abs(indexMinusOne - (samplesMinusOne / 2.0))) / samplesMinusOne);
				#endif
				break;
...
void FFT_handler::set_fft_output_units(int units){
	this->_fft_output_units = units;
}

void FFT_handler::set_fft_input_units(int units){
	this->_fft_input_units = units;
}


void FFT_handler::FFT(bool convert_units,bool normalise_with_window_sample_nr, bool normalise_for_half_window)
{	
