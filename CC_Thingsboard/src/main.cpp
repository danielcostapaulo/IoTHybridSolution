...
constexpr char TOKEN[] = "DeviceToken";
...
//-----------------------------------------------------
  ...
  if(sample_n==SAMPLES-1){
    ...
    if(kmeans_state>=0){
      ...
      if(samples_sent[kmeans_state]<SAMPLES_TO_SEND){
        int next;
        if(buffer_idx+1>=SAMPLES_TO_SEND*2) next=0;
        else next=buffer_idx+1;
        if(next!=last_peak_sent){
      ...
      if(millis() - previousDataSend > telemetrySendInterval && buffer_idx!=last_peak_sent){
        ...
        Serial.print("buffer:");Serial.println(buffer_idx);
        Serial.print("last:");Serial.println(last_peak_sent);
        if(last_peak_sent<(SAMPLES_TO_SEND*2)-1){
          last_peak_sent++;
        }
        else{
          last_peak_sent=0;
