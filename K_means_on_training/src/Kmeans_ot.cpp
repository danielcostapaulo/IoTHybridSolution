...
class Kmeans_OT{
    ...
    private:
        ...
        int make_new_cluster(){
            ...
            float mean=0;
            for(int sample_n=0;sample_n<int(distances.size());sample_n++){
            ...
            for(int i=0;i<distances.size();i++){
                if(distances[i]<mean+std){
                    final_distances.push_back(distances[i]);
                }
            }
            new_max_dist=final_distances[int(final_distances.size()-1)];
            cout<<"did new cluster"<<endl;
/*
            if(clusters.size()>0){
                for(int i=0;i<clusters.size();i++){
                    dist=euclidean_distance(new_cluster,clusters[i]);
                    if(dist<max_dist[i] || dist<new_max_dist){
                        cout<<"cluster "<<i<<" and new cluster"<<endl;
                        max_dist[i]=max_dist[i]*1.1;
                        swicth_to_make_new_cluster=0;
...
int main(){
      ...
      while ((en = readdir(dr)) != NULL) {
        if(en->d_name[0]=='s' || en->d_name[0]=='l'){      
            ...
            while(n_of_features_done<n_of_samples_per_state && n_of_samples_done<int(raw_data.size())){
                FFT_buffer[j]=(raw_data[n_of_samples_done][axis])*10;
                n_of_samples_done++;
                j++;
                if(j==SAMPLES-1){
                    ...
                    for(int k=0;k<SAMPLES;k++){
                        if(FFT_amp[k]!=0) all_axis_FFT[(axis*SAMPLES)+k]=FFT_amp[k];
    ...
    for(int state=0;state<test_DS.size();state++){
        for(int sample_n=0;sample_n<test_DS[state].size();sample_n++){
            ...
            if(result!=cluster_to_state[result]) err++;
            else corr++;
        }
    }
    int cm1[clf.clusters.size()+1][train_DS.size()]={};
    for(int i=0;i<predicted.size();i++){
        if(predicted[i]==-1) cm1[clf.clusters.size()][real[i]]++;
        else if(predicted[i]!=-2) cm1[predicted[i]][real[i]]++;
    }
    for(int i=0;i<clf.clusters.size()+1;i++){
